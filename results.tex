...
In our first experiments we compare unstructured text retrieval with structured text retrieval. For structured text retrieval, we focus on the underlying IMRaD structure. Therefore, each document in the dataset contains a set of all index terms ($DS_{\textit{All}}$). In addition, they contain $5$ sets for each IMRaD-type ($DS_\textit{Introduction}$, $DS_\textit{Background}$, $DS_\textit{Methods}$, $DS_\textit{Results}$, $DS_\textit{Discussion}$). The union of the $5$ sets differs from the set containing all index terms, as there exist areas in the document that cannot be assigned to an IMRaD-type (e.g., Abstract).

To generate index terms stopwords are removed and words are stemmed during the indexing process of an article. Afterwards, the index terms are assigned to document sets. For example, "\textit{I love deadlines.}" is in the \textit{Introduction} and "\textit{I like the whooshing sound they make as they fly by.}" is in the \textit{Methods} of an article. The underlying document sets are given by $DS_{\textit{All}}$ = \{"\textit{love}", "\textit{deadlin}", "\textit{whoosh}", "\textit{sound}", "\textit{fly}"\}, $DS_{\textit{Introduction}}$ = \{"\textit{love}", "\textit{deadlin}"\}, and $DS_{\textit{Methods}}$ = \{"\textit{whoosh}", "\textit{sound}", "\textit{fly}"\}.
%\lstset{language=XML}
%\begin{lstlisting}
%<document>
%  <h1>Introduction</h1>
...
The second best algorithm is \textit{TF}. It achieves an accuracy of $0.1966$ with disabled features and a query length of $11$. In contrast, the same algorithm achieves a MAP of $0.1293$ with enabled features and a query length of $12$.
% split paragraph to prevent formatting issues of the big table
\begin{table}[h]
...
Therefore, unstructured text retrieval is $6.73 \%$ better than structured text retrieval for this configuration. In comparison to \textit{TF-IDF} the achieved accuracy is $2.33 \%$ lower with disabled features, and $3.49 \%$ lower with enabled features. The best performing query lengths are the same as for \textit{TF-IDF}.
