...
After the design of the framework, a ranking function can be created. It should be constructed in a way that it fits at its best to the requirements of the user. This means for a given query the ranking function supply a numeric rank to each document in the collection which represent the relevance for the user.

% todo beispiel term frequercy... what is term frequency... normalized...

% ranked boolean and other models etc.

\section{Ranking Algorithms}
\label{sec:ranking_algorithms}

%IRS has to deal with the problem which documents are relevant and which are not relevant to the user. Hard to decide since each user has different understanding what's relevant, and what's not relevant.. System implement a predictive algorithm which approximates... modern info retrieval page 58
