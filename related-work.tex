...
The vector space model is just one information retrieval model which takes the advantage of term-term correlation into account. Other models would be fuzzy information retrieval models, the set-based model, and the language models.

The vector space model uses term weights for the representation of the vectors. Ribeiro-Neto and Baeza-Yates ~\cite{ModernInvormationRetrieval1999} define the document representation $d_j$, and the query representation $q$ as follows:
\begin{align}
  \vec{d_j} & = (w_{1, j}, w_{2, j}, \dots, w_{t, j}) \\
  \vec{q} & = (w_{1, q}, w_{2, q}, \dots, w_{t, q})
\end{align}
The weight $w_{i,j}$ contains the weight of term $t_i$ in document $d_j$. It has to be non-negative, and non-binary. ...

\myfig{diff_vectors}
      {width=0.5\textwidth}
      {\textbf{Example of the similarity between query and documents in a $2$D space.} Description}
      {Example of the similarity between query and documents in a $2$D space.}
      {fig:diff_vectors}
