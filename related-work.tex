...
\begin{itemize}
  ...
  \item The \textit{Results} section contains the answers to the research questions. In addition, the study results are listed and explained according to the hypothesis. 
...
In \Cref{tbl:imrad_distribution} the IMRaD chapter distribution for \textit{PLOS ONE} is shown. The distribution represents the relation between section and the IMRaD type. For example, $99.90\%$ of the articles has \textit{Introduction} as first section. The other IMRaD types cannot be assigned so clearly to a section. \textit{Method} is mostly assigned to the second or the forth section, \textit{Results} to the second or third section, and \textit{Discussion} to the third or fourth section.
