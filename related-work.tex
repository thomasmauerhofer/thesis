...
To increase the readability within articles it is important to have a common structure. Therefore, if the reader is familiar with the structure of the article it is easier to find the relevant pieces of information. Today the IMRaD (Introduction, Method, Results and Discussion) structure is state-of-the-art structure of scientific papers. Within the structure the chapters contain the following information:
\begin{itemize}
  ...
  \item The \textit{Results} section contains the answers to the research questions. In addition, the study results are listed and interpreted according to the hypothesis. 
  \item The \textit{Discussion} section should interpret the results. Furthermore, they are compared with results of other researchers and future work is discussed.
\end{itemize}

When having a look on the history of scientific articles their structure changed over time. Meadows ~\cite{meadows1985} describes in his work that the first articles appear in the the $17$th century. At this time they are published in the form of descriptive letters and chronologically structured narratives. This format was used for more than $2$ centuries. 

Day ~\cite{robert1989} propose in his work that the first IMRaD-like writing structure was generated by \textit{Louis Pasteur} with his book in \textit{"Etudes sur la Biere"} originally published in $1876$. In the book \textit{Introduction}, \textit{Method}, and \textit{Discussion} can be found although the headings of the section were different. Sollaci and Pereira ~\cite{Sollaci-The-2004} describe in their work that the IMRaD structure began to be adopted in the $1940$s. Furthermore, they propose that it became the standard format for scientific articles in the $1970$s. Today, IMRaD is the standard for all major journals.
...
\begin{table}[!b]
  ...
  \begin{tabular}{l c c c c}
    ...
    \bottomrule
  \end{tabular}
  \caption[IMRaD chapter distribution]{\textbf{IMRaD chapter distribution.} For the IMRaD chapter distribution the \textit{PLOS (Public Library of Science) ONE} journal which consists of $33,782$ articles was analyzed. In this Table the relation between section and the IMRaD type is shown. For example, $99.90\%$ of first sections are Introductions and $51.48\%$ of second sections are Results.}
  \label{tbl:imrad_distribution}
\end{table}

Although the format of IMRaD was introduced there are minor differences in the arrangement of chapters. Bertin et al. ~\cite{bertin2013} analyzed articles of several journals according their IMRaD structure distribution. They used $7$ peer-reviewed academic journals from the Public Library of Science (PLOS), compiled of scientific articles in the fields of biology and medicine. The largest journal is \textit{PLOS ONE}, which contains $33,782$ articles. On average an article of \textit{PLOS ONE} consist of $4.47$ sections, and $90.2\%$ of the sections can be assigned to an IMRaD type.

In table \Cref{tbl:imrad_distribution} the IMRaD chapter distribution for \textit{PLOS ONE} is shown. The distribution represents the relation between section and the IMRaD type. For example, $99.90\%$ of the articles has \textit{Introduction} as first section. The other IMRaD types cannot be assigned so clearly to a section. \textit{Method} is mostly assigned to the second or the forth section, \textit{Results} to the second or third section, and \textit{Discussion} to the third or fourth section.

The structure of articles is commonly used in information retrieval systems as it provides additional information. For example, the IMRaD structure can be leveraged by dividing articles according to their IMRaD sections. Therefore, section headings have to be mapped to IMRaD sections. This is done during the index process by classifying extracted section headings. For example, to use a query like "\textit{network} IN \texttt{Methods}" the methods section has to be known for all articles in the document collection. 

Ahmed and Afzal ~\cite{AhmedA20a} describe in their work that dictionary terms, the template of a paper, and in-text citation are state-of-the-art technologies to map section headings onto logical sections. Furthermore, they proposed a novel approach that employs new features for these technologies.
