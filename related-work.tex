...
\begin{table}[b!]
    ...
    \begin{tabular}{ l c }
      ...
      \bottomrule
    \end{tabular}
  \caption[Variants of TF weight]{\textbf{Variants of TF weight.} There exist $5$ important variants of term frequency weight. First, the binary weight is the simplest form, and only captures if a term occurs in a document. Second, the raw frequency uses the term frequency directly, and can be seen as base for term frequency weighting. Third, the log normalization is an extension of the raw frequency, and uses its logarithm. Forth, the double normalization $0.5$ the weighting value always lies between $0.5$ and $1$. Therefore, the weight is always normalized. Fifth, the double normalization K is a generalization of the double normalization $0.5$.}
...
As the TF-IDF weighting scheme is one of the most popular weighting schemes in information retrieval several variants where proposed over time. Ribeiro-Neto and Baeza-Yates ~\cite{ModernInvormationRetrieval1999} describe in their work the most important forms of term frequency weighting (see \Cref{tbl:tf_variants}), and inverse document weighting (see \Cref{tbl:idf_variants}). Furthermore, they state the best combinations of them to generate different TF-IDF weighting schemes for document term weighting, and query term weighting. 
...
For example, a document is given by $d_1$ = \{"\textit{apple}", "\textit{apple}", "\textit{apple}", "\textit{pear}", "\textit{pear}"\}. When searching for "\textit{pear}" the individual variants of term frequency weighting leads to different results. First, the \textit{binary} weight captures if the term occurs in the document. Therefore the weight of $d_1$ is $1$. Second, the \textit{raw frequency} denotes how often a term occurs in a document. As a result the weight is $2$. Third, the log normalization uses $1$, and adds the logarithm of the \textit{raw frequency}. Hence, the weight is approximately $1.7$. Fourth, the double normalization $0.5$, the \textit{raw frequency}, and the maximum of the \textit{raw frequency} are used. Additionally, the weighting value always lies between $0.5$ and $1$. The result of the document weight is $0.8\overline{3}$. Fifth, the double normalization K is a generalization of the double normalization $0.5$. For example, if K is given by $0.1$ the weighting value lies between $0.1$ and $1$.

To show an example for the inverse document frequency the document collection consists of $d_1$ as defined above, and $d_2$ = \{"\textit{apple}", "\textit{peach}"\}. When searching for "\textit{apple}" all documents of the collection contains the term. Therefore, the term is not useful to differs between the documents of the collection. As a result, the inverse document frequency weight is the lowest possible outcome for each weighting scheme. When searching for "\textit{pear}" just $1$ of $2$ documents contain the term. Therefore, the individual variants of inverse document frequency weighting leads to different results. First, the \textit{unary} variant is always $1$. Second, the \textit{inverse frequency} variant uses $N$ as the number of all documents in the collection. Additional $n_i$ is the document frequency of the query term. The logarithm of these to values leads approximately to $0.7$. Third, the \textit{inverse frequency smooth} also uses $N$ and $n_i$. The only difference is that $1$ is added inside the logarithm. The result of the weight is approximately $1.1$. Fourth, the \textit{inverse frequency max} is almost equal to the \textit{inverse frequency smooth}. The only difference is that it uses the term with the largest document frequency instead of $N$. In our example that is "\textit{apple}", which occurs in every document. Therefore the result is also approximately $1.1$. Fifth, the \textit{probabilistic inverse frequency} is almost equal to the \textit{inverse frequency}. The only difference is that inside the logarithm $N$ is subtracted by $n_i$. For this variant the result is $0$. 
