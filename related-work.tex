...
\]
\end{samepage}
It can be seen that the term-document matrix \textbf{M} has size $3$ x $2$, where rows consists of the terms, and columns consists of the documents. For example, the weight in the first row, and in the first column $w_{1, 1}$ contains the weight of $t_1$ in document $d_1$. The transposed matrix consists the terms and the documents the other way round. By multiplying these $2$ matrices the term-term correlation matrix \textbf{C} is generated. It has size $3$ x $3$ where rows and columns consists of the terms in the vocabulary. For example, the entry in the first row, and in the second column contains the joint co-occurrence of the terms $t_1$ and $t_2$.

The vector space model is just one information retrieval model which takes the advantage of term-term correlation into account. Other models would be the set-based model, fuzzy information retrieval models, and language models.

In the vector space model documents, and queries are represented as vectors in an $t$-dimensional space. The size of $t$ is defined by the size of the vocabulary. As a result each index term stands for one dimension in the vector. Ribeiro-Neto and Baeza-Yates ~\cite{ModernInvormationRetrieval1999} define the document representation $d_j$, and the query representation $q$ as follows:
\begin{align}
  \vec{d_j} & = (w_{1, j}, w_{2, j}, \dots, w_{t, j}) \\
  \vec{q} & = (w_{1, q}, w_{2, q}, \dots, w_{t, q})
\end{align}
The document vector $\vec{d_j}$ consists of the term weights according a document in the collection. Thereby $w_{i,j}$ is defined as the weight of term $t_i$ in document $d_j$. It has to be non-negative, and non-binary. The term weights of the query $w_{i, q}$ consists the weight of term $t_i$, which occurs in query $q$. It has to be greater, or equal $0$.
...
The degree of similarity between a document $d_j$, and a query $q$ is calculated by the cosine of the angle between their underlying vectors (see \Cref{fig:diff_vectors}). The calculation is as follows:
\begin{equation}
  \begin{split}
  ...
  \end{split}
\end{equation}
Thereby $\vec{d_j} \bullet \vec{q}$ is the internal product of the $2$ vectors. The factor $|\vec{d_j}|$ is the norms of the document vector, and $|\vec{q}|$ is the norm of query vector. These norms define the document length, respectively the query length. The norm of the document vector is used to provide the same vector length for all documents. The norm of the query vector does not affect the ranking result since it is the same for all documents in the collection. Singhal et. al discuss in their work ~\cite{SinghalBM96} more sophisticated document length normalization for vector space models.
...
\begin{equation}
  \label{log_term_frequency_weigth}
  tf_{i, j} =
  \begin{cases}
...
\begin{table}[b!]
    ...
    \begin{tabular}{ l c }
      ...
      \bottomrule
    \end{tabular}
  \caption[Variants of IDF weights]{\textbf{Variants of IDF weights.} Describe}
  \label{tbl:idf_variants}
\end{table}
In \Cref{tbl:idf_variants} are the different variants of term inverse document frequency weighing schemes... %desc
...
\begin{table}[b]
    ...
    \begin{tabular}{ c c }
      ...
      \bottomrule
    \end{tabular}
  \caption[Recommended TF-IDF weighting schemes]{\textbf{Recommended TF-IDF weighting schemes.} Describe}
  \label{tbl:recommended_tfidf_weights}
\end{table}
In \Cref{tbl:recommended_tfidf_weights} are recommended combinations of TF-IDF weighting schemes... %desc
