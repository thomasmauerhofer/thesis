...
BM$25$ transfer the strengths of the vector space model to the probabilistic model. It requires additional tuning, but achieves better performance than the vector space model for general document collections. Therefore, it has become to a baseline to evaluate new ranking methods.
...
For a practical implementation of Divergence from Randomness the previously discussed alternatives of the individual parts of the equation (see \Cref{dvr_part1_1}, \Cref{dvr_part1_2}, \Cref{dvr_part2_1}, \Cref{dvr_part2_2}, \Cref{dfv_avg_doclen_1}, and \Cref{dfv_avg_doclen_2}) can arbitrary be combined. Therefore, the best performing variant depends on the given document collection.
...
To have a common structure within articles is important to increase their readability. Therefore, if the reader is familiar with the structure of the article, he will find relevant information fast. Today the IMRaD (Introduction, Method, Results and Discussion) structure is state-of-the-art to design scientific papers. Within the structure the chapters contain the following information:
\begin{itemize}
  \item The \textit{Introduction} section should answer the question why the study was done. Furthermore, it contains research questions, a hypothesis, and a research purpose.  
  \item The \textit{Method} section should answer the questions when, where, and how the study was done. Additionally, it contains information about the used materials.
  \item The \textit{Results} section contains the answers of the research questions. In addition, the study results are denoted and interpreted according to the hypothesis. 
  \item The \textit{Discussion} section should answer what the results mean. Furthermore, they are compared with results of other researchers and future work is discussed.
\end{itemize}

When having a look on the history of scientific articles their structure changed over time. Meadows ~\cite{meadows1985} described in his work that the first articles appears in the the $17$th century. At this time they are published in the form of descriptive letters and chronologically structured narratives. This format was used for more than $2$ centuries. 

Day ~\cite{robert1989} proposed in his work that the first IMRaD-like writing structure was generated by \textit{Louis Pasteur} with his book in \textit{"Etudes sur la Biere"} originally published in $1876$. In the book \textit{Introduction}, \textit{Method} and \textit{Discussion} can be found although the headings of the section were different. Sollaci and Pereira ~\cite{Sollaci-The-2004} describe in their work that the IMRaD structure began to be adopted in the $1940$s. Furthermore, they proposed that it became to the standard format for scientific articles in the $1970$s. Today, IMRaD is the standard for all major journals.

\begin{table}[!b]
  \centering
  \begin{tabular}{l c c c c}
    \toprule
    \textbf{Name of Section} & \textbf{Section $1$} & \textbf{Section $2$} & \textbf{Section $3$} & \textbf{Section $4$} \\ \midrule
    \textbf{Introduction} &  $\mathbf{99.90\%}$ &           $0.07\%$  &           $0.00\%$  &           $0.00\%$  \\ 
    \textbf{Method}       &           $0.00\% $ &          $47.82\%$  &           $5.95\%$  &  $\mathbf{46.86}\%$ \\ 
    \textbf{Results}      &           $0.00\% $ &  $\mathbf{51.48\%}$ &  $\mathbf{47.90\%}$ &           $0.12\%$  \\ 
    \textbf{Discussion}   &           $0.00\% $ &           $0.08\%$  &          $45.79\%$  &          $46.78\%$  \\ 
    \textbf{Other}        &           $0.09\% $ &           $0.55\%$  &           $0.36\%$  &           $6.24\%$  \\ \midrule
    \textbf{Total}        &         $100.00\% $ &         $100.00\%$  &         $100.00\%$  &         $100.00\%$  \\
    \bottomrule
  \end{tabular}
  \caption[IMRaD chapter distribution]{\textbf{IMRaD chapter distribution.} In this Table the relation between section and the IMRaD type is shown. For example, $99.90\%$ of first sections are Introductions and $51.48\%$ of second sections are Results. For the analysis the \textit{PLOS (Public Library of Science) ONE} journal which consists of $33,782$ articles was used.}
  \label{tbl:imrad_distribution}
\end{table}

Although the standardized format of IMRaD was introduced there are minor differences in the arrangement of the chapters. Bertin et al. ~\cite{bertin2013} analysed the articles of several journals according their IMRaD structure distribution. They used $7$ peer-reviewed academic journals from the Public Library of Science (PLOS), where they consist of scientific articles in the field of biology and medicine. The largest journal is \textit{PLOS ONE}, which contains $33,782$ articles. On average a article of \textit{PLOS ONE} consist of $4.47$ sections, and $90.2\%$ of the sections can be assigned to an IMRaD type.

In table \Cref{tbl:imrad_distribution} the proposed IMRaD chapter distribution for \textit{PLOS ONE} is shown. There $99.90\%$ of the articles has \textit{Introduction} as first section. The other IMRaD types can not be applied so clearly to a section. \textit{Method} is mostly assigned to the second or the forth section, \textit{Results} to the second or third section, and \textit{Discussion} to the third or fourth section.

The structure of articles is commonly used in information retrieval systems as it provides additional information. One way to introduce such a structure 
is to divide articles according to their IMRaD sections. Therefore, section headings have to be mapped to IMRaD sections. This is done during the index process by classifying extracted section headings. For example, to use a query like "\textit{network} IN \texttt{Methods}" the methods section has to be known for all articles in the document collection. Ahmed and Afzal ~\cite{AhmedA20a} describe in their work that dictionary terms, the template of a paper, and in-text citation are state-of-the-art technologies to map section headings onto logical sections. Furthermore, they defined a novel approach that employs new features for these technologies.

In Summary, IMRaD is widespread as it has been used for centuries. Due to its clear structure and comprehensibility, it quickly became the standard for scientific articles. This is also the reason why it can be found in every scientific field.