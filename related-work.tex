...
When having a closer look on index terms and their correlations it can be assumed that they are mutually independent. This means knowing $w_{i, j}$, where $w_{i, j}$ is the weight of an index term $k_i$ in a document $d_j$, tells nothing about the next weight $w_{i + 1, j}$. This assumption does not hold, since the terms in a document are always related to each other. One example would be the words \textit{mobile} and \textit{phone}, which often occur together. This means when a document contains the word \textit{mobile} it is probably that the document contains also the word \textit{phone}. This correlation is mirrored by their term weigths. The term-term correlation matrix defined by Ribeiro-Neto and Baeza-Yates ~\cite{ModernInvormationRetrieval1999} reflect such relations. It is defined by... M...

\begin{align}
  c_{u, v} = \sum_{d_j}w_{u, j} \times w_{v, j}
\end{align}
Describe the definition.. transition with 2 documents and 3 terms
%-------------------------------------------------------------------------------
\[
