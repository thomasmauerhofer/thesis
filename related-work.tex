...
\begin{table}[b!]
    ...
    \begin{tabular}{ l c }
      ...
      \bottomrule
    \end{tabular}
  \caption[Variants of TF weight]{\textbf{Variants of TF weight.} There exist $5$ important variants of term frequency weight. The binary weight is the simplest form, and only captures if a term occurs in a document. The raw frequency uses the term frequency directly, and can be seen as base for term frequency weighting. The log normalization is an extension of the raw frequency, and uses its logarithm. For the double normalization $0.5$ the weighting value always lies between $0.5$ and $1$. Therefore, the weight is always normalized. The double normalization K is a generalization of the double normalization $0.5$.}
...
For example, a document is given by $d_1$ = \{"\textit{apple}", "\textit{apple}", "\textit{apple}", "\textit{pear}", "\textit{pear}"\}. When searching for \textit{"pear"} the individual variants of term frequency weighting leads to different results. The \textit{binary} weight captures if the term occurs in the document. Therefore the weight of $d_1$ is $1$. The \textit{raw frequency} denotes how often a term occurs in a document. As a result the weight is $2$. The log normalization uses $1$, and supplies the logarithm of the \textit{raw frequency}. Hence, the weight is approximately $1.7$. For the double normalization $0.5$, the \textit{raw frequency}, and the maximum of the \textit{raw frequency} are used. Additionally, the weighting value always lies between $0.5$ and $1$. The result of the document weight is $0.8\overline{3}$. The double normalization K is a generalization of the double normalization $0.5$. For example, if K is given by $0.1$ the weighting value lies between $0.1$ and $1$.
