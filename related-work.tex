...
\end{equation}
where $f_{i, q}$ is the term frequency of term $k_i$ in query $q$. To approximate the $2$ assumptions of the term weight as described by Amati and Rijsbergen ~\cite{AmatiR02} the following definition are needed:
\begin{align}
  F_i & = \sum_j f_{i, j} \\
  \lambda_i & = p \times F_i,
\end{align}
...
The first term of the term weight (see \Cref{dfr_term_weight}) can be approximated to:
\begin{equation}
  \begin{split}
