...
We define our query in a way that the user can choose if unstructured or structured text retrieval is used. Therefore, we design an IN-statement that connects query terms and IMRaD-Types. For example, with the query "\textit{local}, \textit{network} IN \texttt{Methods}" a user can specify that the terms \textit{local} and \textit{network} should occur in the \texttt{Methods} section of a document. The left part of the IN-statement is converted into a set of query terms. Hence, multiple occurrences of a single term in the query does not affect the outcome. The right part of the IN-statement captures in which set the query term is stored. Hence, with respect to the $5$ IMRaD-Types we defined there are $6$ query sets ($QS_{\textit{All}}$, $QS_{\textit{Introduction}}$, $QS_{\textit{Background}}$, $QS_{\textit{Methods}}$, $QS_{\textit{Results}}$, $QS_{\textit{Discussion}}$). The query sets are connected directly to the documents bags of words in the framework. For example, the terms in $QS_{\textit{Introduction}}$ are used to search in $DBW_{\textit{Introduction}}$.

When a query contains terms without any IN-statement the terms are added to $QS_{\textit{All}}$. Therefore, query terms are stored in an IMRaD query set if an IN-statement is denoted, or in $QS_{\textit{All}}$ otherwise. As a result query terms are searched in $DBW_{\textit{All}}$ or $DBW_\textit{IMRaD}$, but never in both of them. This query structure avoids the usage of unstructured and structured text retrieval with a single request.
...
In our system it is possible to use entire documents (in PDF format) to search for other documents. In this case the user has to specify if unstructured or structured text retrieval is used. The specification influence the generation of the query. The system processes the generated queries in the same way as user-defined queries. To transform a document into a query several steps are necessary: 
\begin{enumerate}
 ...
 \begin{itemize}
  \item For unstructured text retrieval all terms are added to the same set. Afterward, a query is generated by joining together all terms of the set separated by a comma.
  \item For structured text retrieval terms are added to a set with respect to the IMRaD-Type of the section they occur. In the system one set is specified for each IMRaD-Type. Afterward, each set is transformed into an IN-statement. The left part of the statement are the terms of the set, and the right part is the IMRaD-Type of the set. Finally, all IN-statements are joined together with AND-statements to generate the query.  
 \end{itemize} 
\end{enumerate}

In our system \textit{Term Frequency (TF)}, \textit{Term Frequency - Inverse Document Frequency (TF-IDF)}, \textit{BM$25$}, \textit{Divergence from Randomness (DfR)}, and \textit{Ranked Boolean Retrieval (RBR)} are available as ranking algorithms. All of them calculate rankings based on a query $q$, and a document $d_j$ (see \Cref{sec:unstructured_text_Retrieval} for the definition of the algorithms). 

% sim(dj,q)

% define 5 ranking algorithm - state where data comes from...
