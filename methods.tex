...
\end{align}
where IMRaD-TYPES is an array containing all IMRaD-Types. ... %describe formula... d_j + d_{j, k} and q + q_k


%In our system \textit{Term Frequency (TF)}, \textit{Term Frequency - Inverse Document Frequency (TF-IDF)}, \textit{BM$25$}, \textit{Divergence from Randomness (DfR)}, and \textit{Ranked Boolean Retrieval (RBR)} are available as ranking algorithms. 
