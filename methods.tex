...
Index terms are generated by removing stopwords and stemming words during the indexing process of a document. Afterwards, they are assigned to bag of words of a document. For example, "\textit{I love deadlines.}" is in the \textit{Introduction} and "\textit{I like the whooshing sound they make as they fly by.}" is in the \textit{Methods} of a document $A$. The underlying bags of words in the document are given by:
\begin{equation}
...
\begin{enumerate}
 \item First, we extract the article structure and the text content from the PDF. Therefore, we used a framework described in ~\cite{KlampflGJK14}.
