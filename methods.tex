...
\begin{figure}[!t]
  ...
  \begin{minipage}[b]{0.59\textwidth}
    ...
    \end{minipage}
  \end{minipage}
  \caption[In-Degree Distribution of the Citation Network]{\textbf{In-Degree Distribution.} The in-degree distribution descripes how often articles get cited by other articles. The positive screw of the distribution indicates that there are a lot of articles which are cited only a few times, and a few articles which was cited more often. There are some single outliers with a higher degree than 20. This can also be seen by the difference between the mean and the median. The max degree represents that one single article was cited by 98 other articles.}
  \label{fig:indegree_distribution}
\end{figure}

In \Cref{fig:indegree_distribution} the in-degree distribution of our citation network is shown. In generall the in-degree of a node is descriped by the number of ingoing edges. The in-degree distribution represents the probability distribution of these nodes over the whole network. Regarding a citation network the in-degree of a node can be descriped as the number of times an article gets cited by other articles. The positive screw of the in-degree distribution indicates that there are a lot of articles which are cited only a few times, and a few articles which was cited more often. There are only some single articles with an in-degree higher than 20. This is the reason why the median is higher than the mean. The max degree represents that one single article was cited by 98 other articles.
...
The out-degree distribution and their properties are displayed in In \Cref{fig:outdegree_distribution}. How is it represented (citations to papers)
