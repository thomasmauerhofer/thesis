...
To generate index terms stopwords are removed and words are stemmed during the indexing process of a document. Afterwards, the index terms are assigned to document sets. For example, "\textit{I love deadlines.}" is in the \textit{Introduction} and "\textit{I like the whooshing sound they make as they fly by.}" is in the \textit{Methods} of a document. The underlying document sets are given by $DS_{\textit{All}}$ = \{"\textit{love}", "\textit{deadlin}", "\textit{whoosh}", "\textit{sound}", "\textit{fly}"\}, $DS_{\textit{Introduction}}$ = \{"\textit{love}", "\textit{deadlin}"\}, and $DS_{\textit{Methods}}$ = \{"\textit{whoosh}", "\textit{sound}", "\textit{fly}"\}.
